  ...
  return (
    <main>
      <nav className="nav-shell">
        ...
        <div className="nav-links"><a href="#latest">Latest</a><a href="#signals">Field notes</a><a href="#opportunities">Opportunities</a><a href="#contribute">Contribute</a></div>
      ...
      <section className="latest-section" id="latest">
        <div className="section-heading"><div><p className="eyebrow">The reading room / 02</p><h2>Start with the<br /><i>latest source.</i></h2></div><p className="section-description">Short briefs belong here only when their source article has a visible publication date. Until then, the field notes below stay clearly marked as background reading.</p></div>
        <div className="latest-empty"><span className="latest-mark">↗</span><div><p className="story-type">Latest articles / transparent by default</p><h3>No dated articles have been added yet.</h3><p>The index will never quietly turn old material into “today’s news”. Add a dated source through the public contributor desk and it can enter this list after review.</p><a className="arrow-link" href="#contribute">Suggest a dated article <span>↘</span></a></div></div>
        <div className="source-library"><div><p className="story-type">Where to read further</p><p className="library-note">These are starting points, not endorsements. Check the original publication, author, date, methods, and conflicts of interest.</p></div><div className="source-list">{visibleSources.map((source) => <a className="source-item" href={source.link} target="_blank" rel="noreferrer" key={source.name}><strong>{source.name}</strong><span>{source.kind} ↗</span></a>)}<button className="view-more-button" onClick={() => setShowAllSources(!showAllSources)}>{showAllSources ? 'Show less' : 'View more sources'} <span>{showAllSources ? '↟' : '↘'}</span></button></div></div>
      </section>
      <section className="signals-section" id="signals">
        ...
        <div className="signal-grid">
          {visibleStories.map((story, index) => index === 0 && activeTopic === 'All signals' ? <article className={`feature-story ${story.tone}`} key={story.title}><div className="feature-index">01 / BACKGROUND</div><div className="feature-copy"><p className="story-type">{story.type} / {story.topic}</p><h3>{story.title}</h3><p className="story-summary">{story.summary}</p><p className="story-why"><strong>Why it matters</strong>{story.why}</p><p className="story-credit">Prepared by The Bright Index community · Source credit: {story.source}</p><a className="source-link" href={story.link} target="_blank" rel="noreferrer">Read the source ↗</a><button className="story-open" onClick={() => setSelectedStory(story)}>Open the field note <span>↗</span></button></div><div className="feature-orbit" aria-hidden="true"><span>☼</span><i /><i /><b>ADITYA<br />L1</b></div></article> : <article className={`signal-card ${story.tone}`} key={story.title}><p className="story-type">{String(index + 1).padStart(2, '0')} / {story.topic} / BACKGROUND</p><h3>{story.title}</h3><p>{story.summary}</p><p className="story-credit">Community note · Source credit: {story.source}</p><a className="source-link" href={story.link} target="_blank" rel="noreferrer">Open source ↗</a><button className="story-open" onClick={() => setSelectedStory(story)}>Follow the thread <span>↗</span></button></article>)}
        </div>
        <button className="view-more-button section-view-more" onClick={() => setShowAllStories(!showAllStories)}>{showAllStories ? 'Show fewer field notes' : `View more field notes (${filteredStories.length - visibleStories.length})`} <span>{showAllStories ? '↟' : '↘'}</span></button>
      ...
      {selectedStory && <div className="story-modal-backdrop" role="presentation" onClick={() => setSelectedStory(null)}><article className="story-modal" role="dialog" aria-modal="true" aria-labelledby="story-title" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelectedStory(null)} aria-label="Close field note">×</button><p className="eyebrow">{selectedStory.topic} / background field note</p><h2 id="story-title">{selectedStory.title}</h2><p className="modal-summary">{selectedStory.explanation}</p><div className="modal-columns"><div><p className="modal-label">Why it matters</p><p>{selectedStory.why}</p></div><div><p className="modal-label">Follow this rabbit hole</p><p>{selectedStory.rabbitHole}</p></div></div><div className="modal-next"><p className="modal-label">A way to explore further</p><p>{selectedStory.opportunity}</p><p className="story-credit">Prepared by The Bright Index community · Source credit: {selectedStory.source}</p><a className="source-link" href={selectedStory.link} target="_blank" rel="noreferrer">Check the original source ↗</a></div></article></div>}
      <section className="discover-section" id="discover">
        <div className="section-heading"><div><p className="eyebrow">The people / 01</p><h2>Celebrate the<br /><i>minds behind it.</i></h2></div><p className="section-description">Meet the scientists and builders whose questions, experiments, and persistence are expanding what India can do.</p></div>
      ...
      <section className="opportunity-section" id="opportunities">
        <div className="section-heading"><div><p className="eyebrow">The next step / 04</p><h2>Curiosity needs<br /><i>a way forward.</i></h2></div><p className="section-description">A separate layer for fellowships, labs, courses, and projects that turn interest into practice.</p></div>
