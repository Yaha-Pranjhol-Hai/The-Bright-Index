  ...
  return (
    <main>
      ...
      <section className="latest-section" id="latest">
        ...
        <div className="source-library"><div><p className="story-type">Where to read further</p><p className="library-note">These are starting points, not endorsements. Check the original publication, author, date, methods, and conflicts of interest.</p></div><div className="source-list">{readingSources.map((source) => <a className="source-item" href={source.link} target="_blank" rel="noreferrer" key={source.name}><strong>{source.name}</strong><span>{source.kind} ↗</span></a>)}</div></div>
      </section>
      <section className="signals-section" id="signals">
        <div className="section-heading signals-heading"><div><p className="eyebrow">The field notes / 02</p><h2>What India is<br /><i>learning over time.</i></h2></div><p className="section-description">Durable explainers for people who want more than a headline. Each one points to a source and a path for going deeper.</p></div>
